...
export default function Preview() {
  ...
  const {movies} = useSelector(data => data.movies); // currect data from redux
   ...
   <section className='h-[56vw] xl:h-[100%] flex pt-[25%] px-12 flex-col bg-center bg-cover mb-6 bg-default pb-4 md:px-6' 
    style={{backgroundImage:`url("https://image.tmdb.org/t/p/original/${movies?.backdrop_path}")`}}>
        <div className='flex flex-col mb-6'>
               <h1 className='text-8xl lg:text-6xl mb-5 lit:text-4xl'>{movies?.title || movies?.name || movies?.original_name}</h1>
               <h2 className='max-w-[400px] max-h-32 md:text-sm'>{movies.overview === undefined ? ""  : (movies.overview.length > 150 ? movies.overview.substring(0,150)+"...": movies.overview)}</h2>
        </div>
